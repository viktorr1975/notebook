...
#создадим пользователя 'user21' с двумя заметками и тэгом к этим заметкам
        ...
        self.tag_user21 = Tags.objects.create(
...
# создадим пользователя 'user20' с одной заметкой
    ...
    def test_anonymous_cannot_see_page(self):   #проверяме отсутствие доступа GET к странице "/api/notes" неавторизованного пользователя
        response = self.client.get(reverse("notesapp:notes-list"))
        self.assertEqual(401, response.status_code) #Unauthorized

    def test_authenticated_user_can_see_page_home(self): #проверяме наличия доступа GET к странице "/api/notes" авторизованного пользователя
        #self.client.login(username='fred', password='secret')  #test client will have all the cookies and session data
        self.client.force_login(user=self.user20)   #test requires a user be logged in and the details of how a user logged in aren’t important
#        print("resolved name:", resolve("/api/notes").url_name)
        ...
        self.assertEqual(1, resp_json["count"])         #в запросе предоставлена только единственная запись пользователя user20
        self.client.logout()
...
# создадим пользователя 'user20' с одной заметкой
    ...
    def test_authenticated_user_can_see_page_home(self): #проверяме наличие доступа к странице "/" авторизованного пользователя
        #self.client.login(username='fred', password='secret')  #test client will have all the cookies and session data
        self.client.force_login(user=self.user20)   #test requires a user be logged in and the details of how a user logged in aren’t important
...
#        self.assertTemplateUsed(response, '_index.html')
    ...
    def test_anonymous_cannot_see_page_home(self):   #проверяме отсутствие доступа к странице "/" неавторизованного пользователя
        # url = reverse('home')
        # print("Resolve : ", resolve(url))
        response = self.client.get(reverse("home"))
        self.assertRedirects(response, "/accounts/login/?next=/")

    def test_anonymous_cannot_see_page_detail(self):   #проверяме отсутствие доступа к странице "category/<int:group_id>" неавторизованного пользователя
        response = self.client.get(reverse("detail", args=[1]))     #значение args в этом тесте роли не играет
        print(response)
